...
ir = 0
emo = 0
laugh = ['lol', 'haha', 'laugh', ':D', '=D', 'lmao', 'hehe', 'lmfao', 'rotfl', 'rotflol', 'grin']
for i in xrange(train_len):
    # if Y_train[i] == 1 and c < 15:
    #     print "Sentence ", i
    #     print X_train[i]
    #     c += 1
    # if any(l in X_train[i] for l in laugh):
    if Y_train[i] == 1:
        ir += 1
    if ":" in X_train[i]:
        if ("http:" not in X_train[i] and "https:" not in X_train[i]):
            if c < 15:
                print "Sentence ", i
                print X_train[i]
            c += 1
    if re.search(":\w+:", X_train[i]):
        emo += 1
print "c = ", c
print "emo = ", emo
print train_len
